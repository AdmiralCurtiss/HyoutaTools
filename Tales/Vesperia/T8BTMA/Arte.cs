...
namespace HyoutaTools.Tales.Vesperia.T8BTMA {
	public class Arte {
		...
		public string GetDataAsHtml( GameVersion version, Dictionary<uint, Arte> arteIdDict, T8BTEMST.T8BTEMST enemies, T8BTSK.T8BTSK skills, TSS.TSSFile stringDic, Dictionary<uint, TSS.TSSEntry> inGameIdDict ) {
			StringBuilder sb = new StringBuilder();
			sb.Append( "<tr id=\"arte" + ID + "\">" );
			...
			if ( Character > 9 ) {
				...
				sb.Append( "<img src=\"monster-icons/48px/monster-" + enemy.IconID.ToString( "D3" ) + ".png\" width=\"32\" height=\"32\" title=\"" + inGameIdDict[enemy.NameStringDicID].StringEngOrJpn + "\">" );
				sb.Append( "</a>" );
			}
			...
			sb.Append( VesperiaUtil.RemoveTags( inGameIdDict[NameStringDicId].StringJPN, true, true ) + "</span><br>" );
			sb.Append( VesperiaUtil.RemoveTags( inGameIdDict[DescStringDicId].StringJPN, true, true ).Replace( "\n", "<br>" ) );
			sb.Append( "</td>" );
			sb.Append( "<td>" );
			sb.Append( "<span class=\"itemname\">" );
			sb.Append( inGameIdDict[NameStringDicId].StringENG + "</span><br>" );
					...
					switch ( LearnRequirementsOtherArtesType[i] ) {
						...
						case 2: // Other Arte
							...
							if ( otherArte.ID != this.ID ) {
								...
								sb.Append( "<a href=\"#arte" + otherArte.ID + "\">" + inGameIdDict[otherArte.NameStringDicId].StringEngOrJpn );
								sb.Append( "</a>, " );
							} else {
				...
				if ( AlteredArteRequirementType[i] > 0 ) {
					switch ( AlteredArteRequirementType[i] ) {
						case 1: // original arte
							...
							sb.Append( "<img src=\"menu-icons/artes-" + otherArte.GetIconNumber() + ".png\" width=\"16\" height=\"16\">" );
							sb.Append( "<a href=\"#arte" + otherArte.ID + "\">" + inGameIdDict[otherArte.NameStringDicId].StringEngOrJpn + "</a>" );
							break;
						case 3: // skill
