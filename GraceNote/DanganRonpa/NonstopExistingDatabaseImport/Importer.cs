		...
		public static int AutoImport() {
			...
			foreach ( var x in nonstopDict ) {
				...
				foreach ( var item in nonstop.items ) {
					...
					string charac = DanganUtil.CharacterIdToName( (byte)item.data[(int)NonstopSingleStructure.Character] );
					if ( charac == "Naegi" ) { charac = "Neagi"; }
					string voicename = "[" + charac + "] " + item.data[(int)NonstopSingleStructure.Chapter] + " "
