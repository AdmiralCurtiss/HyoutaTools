...
namespace HyoutaTools.Tales.Vesperia.T8BTEMEG {
	public class T8BTEMEG {
		public T8BTEMEG( String filename, EndianUtils.Endianness endian, BitUtils.Bitness bits ) {
		...
		public T8BTEMEG( Stream stream, EndianUtils.Endianness endian, BitUtils.Bitness bits ) {
			if ( !LoadFile( stream, endian, bits ) ) {
				throw new Exception( "Loading T8BTEMEG failed!" );
			}
		}
